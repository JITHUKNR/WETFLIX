...
def process_media_request(message, db_collection, send_function, error_text):
    ...
    # ടൈമർ പരിശോധന
    if user_id != ADMIN_ID: # അഡ്മിന് ടൈമർ ബാധകമല്ല
...
# 1. ബ്രോഡ്കാസ്റ്റ് സിസ്റ്റം (/broadcast <മെസ്സേജ്>)
...
def admin_broadcast(message):
    ...
    for u in users:
        try:
            ...
            success += 1
            time.sleep(0.05) # ടെലിഗ്രാം സ്പാം ഒഴിവാക്കാൻ ചെറിയൊരു ഇടവേള
        except:
            failed += 1
            
    bot.reply_to(message, f"✅ ബ്രോഡ്കാസ്റ്റ് പൂർത്തിയായി!\nവിജയകരം: {success}\nപരാജയം: {failed}")

# 2. യൂസർ സ്റ്റാറ്റിസ്റ്റിക്സ് (/stats)
...
def admin_stats(message):
    ...
    text = f"📊 **ബോട്ട് സ്റ്റാറ്റിസ്റ്റിക്സ്**\n\n" \
           ...
           f"🚫 ബാൻ ചെയ്യപ്പെട്ടവർ: {banned_users}\n\n" \
           f"📁 സേവ് ചെയ്ത വീഡിയോകൾ: {vid_count}\n" \
           f"🖼️ സേവ് ചെയ്ത ഫോട്ടോകൾ: {img_count}\n" \
           f"🎭 സേവ് ചെയ്ത സ്റ്റിക്കറുകൾ: {stk_count}"
    bot.reply_to(message, text)

# 3. ബാൻ സിസ്റ്റം (/ban <ID> & /unban <ID>)
...
# 4. മെയിൻ്റനൻസ് മോഡ് ഓൺ/ഓഫ് (/maintenance)
...
@app.route('/')
def home():
    return "Ultimate Bot is running!"
